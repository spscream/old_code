# -*- coding: utf-8 -*-
"""
AnotherTrac: The code managing the database setup and upgrades.
"""
...
schema = [
	...
	Table('anothertrac_tickets', key = 'ticket')[
	    Column('ticket'),
	    Column('another_trac'),
...
def create_tables(cursor, env):
    ...
    for table in schema:
        for stmt in to_sql(env, table):
            cursor.execute(stmt)
    cursor.execute("INSERT into system values ('anothertrac_version', '1')")
    cursor.execute("INSERT into system values ('anothertrac_infotext', '')")
...
class AnotherTracSetup(Component):
    ...
    def _get_version(self, cursor):
        try:
            sql = "SELECT value FROM system WHERE name='anothertrac_version'"
            self.log.debug(sql)
            cursor.execute(sql)
            for row in cursor:
                return int(row[0])
